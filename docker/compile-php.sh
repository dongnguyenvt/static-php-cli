...
function php_compile_args() {
    ...
    _php_arg="$_php_arg --disable-ipv6"
    _php_arg="$_php_arg --disable-gcc-global-regs"
    _php_arg="$_php_arg --disable-zend-signals"
    _php_arg="$_php_arg --enable-debug=no"
...
export CFLAGS='-Os -s -flto -static'
export CXXFLAGS='-Os -s -flto -static'
...
# 编译 PHP
echo "Compiling PHP xxx ..."
php_dir=$(find $self_dir/source -name "php-$VER_PHP" -type d | tail -n1)
cd $php_dir && \
    ./buildconf --force && \
    LDFLAGS=-static ./configure --prefix=$OUT_DIR/php $(php_compile_args $VER_PHP) && \
